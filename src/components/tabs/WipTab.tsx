...
/** UI selection; "Both" shows iOS and Android wip prompts together. */
...
/**
 * "Wip" tab: prompts in the "wip" (work-in-progress) state. Rows mirror the
 * Ready tab's: a platform icon, the prompt text, and edit/copy/delete
 * actions. Editing here upserts in place and keeps the prompt wip.
 */
export default function WipTab() {
  ...
  // Only wip prompts; cross-platform GET filtered client-side.
  ...
  const commitEdit = async (p: Prompt) => {
    ...
    setError(null)
    try {
      // Upsert keyed on the existing prompt_id; state stays 'wip'.
  ...
  // reloaded, the wip filter hides it from this list.
  ...
  return (
    <section className="prompts-tab">
      ...
      {!loading && !error && visible.length === 0 && (
        <p className="prompts-empty">No wip prompts.</p>
      )}
